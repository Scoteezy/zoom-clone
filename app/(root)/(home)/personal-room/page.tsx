...
const PersonalRoom = () => {
  ...
  const {call} = useGetCallBtyId(meetingId!);
  ...
  return (
    <section className="flex size-full flex-col gap-10 text-white">
      ...
      <div className="flex w-full flex-col gap-8 xl:max-w-[900px]">
        <Table description={`${user?.username}'s meeting Room`}
          title="Topic"/>
        <Table description={`${meetingId!}`}
          title="Meeting Id"/>
        <Table description={`${meetingLink}`}
          title="Invite Link"/>
        <div className="flex gap-5">
          <Button className="bg-blue-1"
            onClick={startRoom}>Start Meeting</Button>
